...
### 17. Pre-release methodology quality checks

Pre-release quality controls now emphasize historical-data-derived forecast defaults when financial statements are available. Forecast seeding calculates three-year revenue CAGR, average and latest EBITDA margin, average D&A/revenue, historical CAPEX/revenue with D&A fallback, and NWC/revenue from historical ratios. Diagnostics were expanded to flag aggressive revenue growth, WACC outside SME DCF review bands, high terminal-value reliance at 75% and 85% thresholds, stale ERP and beta seed datasets, negative NWC ratios, and cases with fewer than three historical years available.
