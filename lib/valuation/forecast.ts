...
export function calculateHistoricalForecastSeed(input: ValuationInput): HistoricalForecastSeed {
  ...
  const rawRevenueCagr = oldestRevenueYear && latestRevenueYear && oldestRevenueYear.revenue > 0 && latestRevenueYear.revenue > 0 && revenueYears.length > 1
    ...
    : 0;
  const bizRaportRevenueCagrReference = [...historicalsByYear].reverse().find((year) => typeof year.revenueCagr3y === "number")?.revenueCagr3y;
  const normalizedBizRaportRevenueCagr = typeof bizRaportRevenueCagrReference === "number" && Number.isFinite(bizRaportRevenueCagrReference)
    ? (Math.abs(bizRaportRevenueCagrReference) > 1 ? bizRaportRevenueCagrReference / 100 : bizRaportRevenueCagrReference)
    : null;
  const canUseHistoricalCagr = Number.isFinite(rawRevenueCagr) && revenueYears.length >= 3;
  const revenueCagrBeforeClamp = canUseHistoricalCagr ? rawRevenueCagr : normalizedBizRaportRevenueCagr ?? 0;
  const revenueCagr = Number.isFinite(revenueCagrBeforeClamp) ? clamp(revenueCagrBeforeClamp, -0.15, 0.15) : 0;

  if (normalizedBizRaportRevenueCagr !== null) {
    notes.push("Generated from imported historical financials; BizRaport 3Y CAGR reference available.");
  }

  if (Number.isFinite(revenueCagrBeforeClamp) && revenueCagrBeforeClamp !== revenueCagr) {
    notes.push("Revenue CAGR was clamped to the -15% to +15% forecast seeding range.");
  }
  const ebitdaMarginYears = historicalsByYear.filter((year) => year.revenue > 0 && Number.isFinite(year.ebitda));
  const ebitdaMargins = ebitdaMarginYears.map((year) => year.ebitda / year.revenue);
  const latestEbitdaMarginYear = [...ebitdaMarginYears].reverse()[0];
  const latestEbitdaMargin = latestEbitdaMarginYear ? latestEbitdaMarginYear.ebitda / latestEbitdaMarginYear.revenue : 0;
