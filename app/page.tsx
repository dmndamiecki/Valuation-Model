...
import { deriveBridgeFromImportedFinancials, deriveWorkingCapitalFromImportedFinancials, type ImportedBridgeDerivation, type ImportedWorkingCapitalDerivation } from "@/lib/valuation/balance-sheet-import";
...
function datasetAgeLabel(date: string | null | undefined) {
  if (!date) {
    return "Unavailable";
  }
  ...
  const days = Math.max(0, Math.floor((Date.now() - parsed.getTime()) / (24 * 60 * 60 * 1000)));
  return `${days} days`;
}
...
function BizRaportFinancialPreview({ data, currency, onApply }: { data: CompanyFinancialData; currency: string; onApply: () => void }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
      ...
      <DataPointRow label="Debt ratio" dataPoint={latest?.debtRatio} formatter={(value) => pct(Number(value))} />
      <DataPointRow label="ROA" dataPoint={latest?.roa} formatter={(value) => pct(Number(value))} />
      <DataPointRow label="ROE" dataPoint={latest?.roe} formatter={(value) => pct(Number(value))} />
      <DataPointRow label="Revenue CAGR 3Y" dataPoint={latest?.revenueCagr3y} formatter={(value) => pct(Math.abs(Number(value)) > 1 ? Number(value) / 100 : Number(value))} />
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          {preview.seed ? (
            <div className="mt-3 space-y-2 text-sm">
              ...
              <OutputRow label="Average EBITDA margin" value={`${pct(preview.seed.ebitdaMargin)} · ${preview.seed.source}`} />
              <OutputRow label="Latest EBITDA margin" value={`${pct(preview.seed.latestEbitdaMargin)} · ${preview.seed.source}`} />
  ...
  return (
    <Card>
      ...
      <CardContent className="grid gap-3 text-sm md:grid-cols-2 xl:grid-cols-3">
        ...
        <OutputRow label="Assets" value={summary.assets !== null ? currencyFormatter.format(summary.assets) : "Unavailable"} />
        <OutputRow label="Equity" value={summary.equity !== null ? currencyFormatter.format(summary.equity) : "Unavailable"} />
        <OutputRow label="Liabilities" value={summary.liabilities !== null ? currencyFormatter.format(summary.liabilities) : "Unavailable"} />
        <OutputRow label="Debt ratio" value={summary.debtRatio !== null ? pct(summary.debtRatio) : "Unavailable"} />
        <OutputRow label="ROA" value={summary.roa !== null ? pct(summary.roa) : "Unavailable"} />
        <OutputRow label="ROE" value={summary.roe !== null ? pct(summary.roe) : "Unavailable"} />
        <OutputRow label="Employees" value={summary.employees !== null ? summary.employees.toFixed(0) : "Unavailable"} />
        <OutputRow label="BizRaport revenue CAGR 3Y" value={summary.revenueCagr3y !== null ? pct(Math.abs(summary.revenueCagr3y) > 1 ? summary.revenueCagr3y / 100 : summary.revenueCagr3y) : "Unavailable"} />
...
export default function Home() {
  ...
  const [importedBridgeDerivation, setImportedBridgeDerivation] = useState<ImportedBridgeDerivation | null>(null);
  const [importedWorkingCapitalDerivation, setImportedWorkingCapitalDerivation] = useState<ImportedWorkingCapitalDerivation | null>(null);
  ...
  const model = useMemo(() => {
    ...
    const diagnostics = calculateValuationDiagnostics(input, {
      erpDatasetAgeDays: erpSource?.datasetAgeDays,
      betaDatasetAgeDays: betaSource?.datasetAgeDays,
      importedLiabilities: importedBridgeDerivation?.importedLiabilities,
      liabilitiesProxyUsed: importedBridgeDerivation?.liabilitiesProxyUsed,
      cashUnavailable: importedBridgeDerivation?.cashUnavailable,
      nwcTotalLiabilitiesProxyUsed: importedWorkingCapitalDerivation?.totalLiabilitiesProxyUsed,
    });
    ...
    const qualityWarnings = diagnostics.diagnostics.filter((diagnostic) => diagnostic.severity !== "info");
    return { forecastYears, wacc, dcf, bridge, discounts, executiveSummary, terminalBreakdown, evToEquityBridge, privateCompanyAdjustmentBridge, warnings, qualityWarnings, scenarioAnalysis, diagnostics, waccCases, growthCases, sensitivity, valuationReport };
  }, [input, erpSource, betaSource, importedBridgeDerivation, importedWorkingCapitalDerivation]);
  ...
  function importedYearsToHistoricals(data: CompanyFinancialData | null, baseInput: ValuationInput) {
    ...
    return baseInput.historicals.map((historical, index) => {
      ...
      const workingCapitalDerivation = deriveWorkingCapitalFromImportedFinancials(imported);
      return {
        ...historical,
        year: imported.year,
        ...
        netWorkingCapital: typeof imported.netWorkingCapital?.value === "number" ? imported.netWorkingCapital.value : workingCapitalDerivation.estimatedNwc ?? historical.netWorkingCapital,
        ebit: typeof imported.ebit?.value === "number" ? imported.ebit.value : undefined,
        netIncome: typeof imported.netIncome?.value === "number" ? imported.netIncome.value : undefined,
        assets: typeof imported.assets?.value === "number" ? imported.assets.value : undefined,
        fixedAssets: typeof imported.fixedAssets?.value === "number" ? imported.fixedAssets.value : undefined,
        currentAssets: typeof imported.currentAssets?.value === "number" ? imported.currentAssets.value : undefined,
        equity: typeof imported.equity?.value === "number" ? imported.equity.value : undefined,
        liabilities: typeof imported.liabilities?.value === "number" ? imported.liabilities.value : undefined,
        debtRatio: typeof imported.debtRatio?.value === "number" ? imported.debtRatio.value : undefined,
        roa: typeof imported.roa?.value === "number" ? imported.roa.value : undefined,
        roe: typeof imported.roe?.value === "number" ? imported.roe.value : undefined,
        employees: typeof imported.employees?.value === "number" ? imported.employees.value : undefined,
        revenueCagr3y: typeof imported.revenueCagr3y?.value === "number" ? imported.revenueCagr3y.value : undefined,
      };
    });
  }
  ...
  function buildImportedValuationInput(profile: CompanyProfileData | null, data: CompanyFinancialData | null, baseSimpleInput: SimpleModeInput = wizardInput) {
    ...
    const latestImportedFinancialYear = data?.years[0];
    const bridgeDerivation = deriveBridgeFromImportedFinancials(latestImportedFinancialYear);
    const workingCapitalDerivation = deriveWorkingCapitalFromImportedFinancials(latestImportedFinancialYear);
  ...
  function buildImportedDataSummary(profile: CompanyProfileData | null, data: CompanyFinancialData | null, suggestion: PkdIndustrySuggestion | null, forecastGenerated: boolean): ImportedDataSummary {
    ...
    return {
      ...
      assets: typeof latest?.assets?.value === "number" ? latest.assets.value : null,
      equity: typeof latest?.equity?.value === "number" ? latest.equity.value : null,
      liabilities: typeof latest?.liabilities?.value === "number" ? latest.liabilities.value : null,
      debtRatio: typeof latest?.debtRatio?.value === "number" ? latest.debtRatio.value : null,
      roa: typeof latest?.roa?.value === "number" ? latest.roa.value : null,
      roe: typeof latest?.roe?.value === "number" ? latest.roe.value : null,
      employees: typeof latest?.employees?.value === "number" ? latest.employees.value : null,
      revenueCagr3y: typeof latest?.revenueCagr3y?.value === "number" ? latest.revenueCagr3y.value : null,
  ...
  function importCombinedCompanyData() {
    ...
    setForecastSeedNotes([...(imported.seed?.notes ?? []), ...imported.bridgeDerivation.notes, ...imported.bridgeDerivation.warnings, ...imported.workingCapitalDerivation.notes, ...imported.workingCapitalDerivation.warnings]);
    setForecastAutoSeeded(Boolean(imported.seed));
    setImportedBridgeDerivation(imported.bridgeDerivation);
    setImportedWorkingCapitalDerivation(imported.workingCapitalDerivation);
  ...
  function applyImportedCompanyData() {
    ...
    if (mode === "simple") {
      ...
      const baseInput = buildValuationInputFromSimpleMode(nextSimpleInput);
      const inputWithImportedHistoricals: ValuationInput = {
        ...baseInput,
      ...
      const bridgeDerivation = deriveBridgeFromImportedFinancials(companyData.years[0]);
      const workingCapitalDerivation = deriveWorkingCapitalFromImportedFinancials(companyData.years[0]);
      const inputWithBridge: ValuationInput = { ...inputWithImportedHistoricals, bridge: { ...inputWithImportedHistoricals.bridge, ...bridgeDerivation.bridge } };
      const seeded = seedForecastFromHistoricals(inputWithBridge);
      setSimpleInput({ ...nextSimpleInput, cash: seeded.input.bridge.cash, debt: seeded.input.bridge.debt, expectedAnnualRevenueGrowth: seeded.input.forecast.revenueGrowth[0], expectedEbitdaMargin: seeded.input.forecast.ebitdaMargin[0] });
      setForecastSeedNotes([...seeded.seed.notes, ...bridgeDerivation.notes, ...bridgeDerivation.warnings, ...workingCapitalDerivation.notes, ...workingCapitalDerivation.warnings]);
      setForecastAutoSeeded(true);
      setImportedBridgeDerivation(bridgeDerivation);
      setImportedWorkingCapitalDerivation(workingCapitalDerivation);
      ...
      setInput((current) => ({
        ...
        historicals: importedYearsToHistoricals(companyData, current),
        bridge: {
          ...current.bridge,
          ...bridgeDerivation.bridge,
        },
      ...
      setImportedBridgeDerivation(bridgeDerivation);
      setImportedWorkingCapitalDerivation(workingCapitalDerivation);
      setForecastSeedNotes((current) => [...current, ...bridgeDerivation.notes, ...bridgeDerivation.warnings, ...workingCapitalDerivation.notes, ...workingCapitalDerivation.warnings]);
    }
  ...
  function startNewValuation() {
    ...
    setImportedBridgeDerivation(null);
    setImportedWorkingCapitalDerivation(null);
  }
  ...
  const marketDataSourceRows = [
    {
      ...
      datasetAge: riskFreeRateSource?.value !== null && riskFreeRateSource?.value !== undefined ? datasetAgeLabel(riskFreeRateSource.observationDate ?? riskFreeRateSource.fetchedAt) : "Manual",
    ...
    },
    {
      ...
      datasetAge: erpSource?.value !== null && erpSource?.value !== undefined ? `${erpSource.datasetAgeDays} days` : "Manual",
    ...
    },
    {
      ...
      datasetAge: betaSource?.value !== null && betaSource?.value !== undefined ? `${betaSource.datasetAgeDays} days` : "Manual",
      ...
      <section className="mx-auto max-w-7xl space-y-8">
            ...
            <Card className="border-teal-200 bg-teal-50/40">
              ...
              <CardContent className="space-y-5">
                ...
                <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                  ...
                  {model.warnings.length > 0 || model.qualityWarnings.length > 0 ? (
                    <ul className="mt-3 space-y-2 text-sm text-amber-950">
                      {model.warnings.slice(0, 3).map((warning) => <li key={warning.code}>{warning.message}</li>)}
                      {model.qualityWarnings.slice(0, 3).map((diagnostic) => <li key={diagnostic.code}>{diagnostic.area}: {diagnostic.message}</li>)}
                    </ul>
                  ) : (
                  ...
                  <tbody>
                    {marketDataSourceRows.map((row) => (
                      <tr key={row.input} className="border-b border-slate-100 align-top">
                        ...
                        <td className="p-3 text-slate-600">{row.datasetAge}</td>
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm">
                ...
                {riskFreeRateSource ? (
                  <div className="mt-3 grid gap-2 text-xs text-slate-600 sm:grid-cols-2">
                    ...
                    <span><strong>Source date:</strong> {riskFreeRateSource.observationDate ?? "Unavailable"}</span>
                    <span><strong>Dataset age:</strong> {datasetAgeLabel(riskFreeRateSource.observationDate ?? riskFreeRateSource.fetchedAt)}</span>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          ...
          <Card>
            ...
            <CardContent className="grid gap-3">
              ...
              {importedBridgeDerivation ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-900">
                <p className="font-semibold">BizRaport bridge source notes</p>
                <p>Cash: {importedBridgeDerivation.cashUnavailable ? "unavailable from BizRaport; editable manual field remains available" : "imported from BizRaport"}</p>
                <p>Other debt-like items: {importedBridgeDerivation.liabilitiesProxyUsed ? "populated from total liabilities proxy; review before finalizing" : "unavailable / not forced"}</p>
                {[...importedBridgeDerivation.notes, ...importedBridgeDerivation.warnings].map((note) => <p key={note}>{note}</p>)}
              </div> : null}
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            ...
            <Card className={model.warnings.length > 0 || model.qualityWarnings.length > 0 ? "border-amber-300 bg-amber-50" : "border-emerald-200 bg-emerald-50"}>
              <CardHeader><CardTitle>Warnings</CardTitle><CardDescription>Automated DCF, private-company, diagnostics, and source-age checks.</CardDescription></CardHeader>
              <CardContent>
                {model.warnings.length > 0 || model.qualityWarnings.length > 0 ? (
                  <ul className="space-y-3 text-sm text-amber-950">
                    ...
                    {model.qualityWarnings.map((diagnostic) => <li key={diagnostic.code} className="flex gap-2"><AlertCircle className="mt-0.5 shrink-0" size={16} /><span>{diagnostic.area}: {diagnostic.message}</span></li>)}
                  </ul>
                ) : (
